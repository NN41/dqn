...
class QNetwork(nn.Module):
    """
    Implements Deep Q-Network using two convolutional layers followed by two fully-connected layers.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor: # input must be of shape (batch_size, 4, 84, 84) or (4, 84, 84)
        h1 = F.relu(self.conv1(x))
        h2 = F.relu(self.conv2(h1))
        h2_flat = torch.flatten(h2, start_dim=-3)
        h3 = F.relu(self.fc1(h2_flat))
        output = self.fc2(h3) # these are not logits, they are predicted Q-values for each individual action
        return output
